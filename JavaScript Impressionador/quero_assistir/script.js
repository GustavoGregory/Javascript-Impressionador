...
async function searchButtonClinckHandler() {
  try {
    let url = `http://www.omdbapi.com/?apikey=${key}&t=${movieNameParamaterGenerator()}${movieYearParamaterGenerator()}
        `;
...
function movieYearParamaterGenerator() {
  if (movieYear.value === "") {
    return "";
  }
...
function addToList(data) {
  if (isFilmAlredyOnTheList(data.imdbID)) {
    notie.alert({ type: "error", text: "Filme já está em sua lista!" });
    return;
  }
  movieList.push(data);
  updateLocalStorage();
...
$searchButton.addEventListener("click", searchButtonClinckHandler);
